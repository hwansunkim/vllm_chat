...
- **전원 침묵** (next_wave가 빔) — 그 순간 활성이고 상태에 안 묶인 에이전트 **전원**을
  ...
  **2 이상이면 위 `idle` 시간 점프** — 가족이 각자 흩어진 낮·각자 잠든 밤이 몇 분씩
  ...
  시점까지 점프한다. 일부만 묶였으면 idle 스케줄 점프가 그 사람의 해제 시점에서
  멈춘다(아래 에이전트 상태 절).
...
3. **"전원 재투입" 시간 점프**(runner.py) — 활성 에이전트 전원이 상태에 묶여
   ...
   않는 것과 대칭 — 이건 반대로 "그 시점까지는 확실히 건너뛴다"). 일부만 묶여
   있으면 idle 스케줄 값을 쓰되 그 해제 시점을 **넘지 않게** 자른다(아래 "자연
   만료" 참고).
...
**자연 만료 — 본인 해제 알림 + 그 wave 턴(사용자 설계).** 상태에 **들어가는 것은
에이전트의 선택**(`enter_state`·zone 경계 이동)이지만 **지속 시간은 엔진이 정한다**.
그러니 끝나는 순간도 엔진이 본인에게 알려야 한다. 예전엔 만료 처리가 LLM 호출이
끝난 **뒤**에 있어서(만료 판정은 wave 시작 시각 기준인데 처리만 늦음) 풀린 본인은
그 wave에 턴을 못 받았고, 본인에게는 이동 중 놓친 메시지가 있을 때만 그게 돌아갔다.
그래서 누가 말을 걸거나 소외/전원 침묵 재투입에 걸릴 때까지 멈춰 있었다 — 실측
(case1): 아빠가 23:09에 씻기가 끝났는데 01:46에야 "씻고 나와 수건으로 닦는다"로
턴을 받음. 이제:

- 만료 처리(`_expire_agent_states`)는 wave **시작 시점**, 예약 이벤트·활성 필터 뒤,
  `if not current_wave` 가드·디렉터·traveling 관문 앞에서 돈다(위치 이유는
  `docs/simulation-engine.md §2` 2b).
- 활성인 만료자 본인은 그 wave의 발화자로 편입되고, incoming은
  `[해제 알림, *이동 중 놓친 메시지(held_incoming), *이미 받은 incoming]` 순이다.
  알림 문구는 `_status_release_notice`(status.py) — 기존 씬 문구("[씬] X이(가) 이곳에
  도착했다.")와 같은 짧은 과거형:
  - `traveling` → `[씬] 고등학교에 도착했다.` (외부 공간 도착이어도 본인 알림은 준다)
  - `sleep` → `[씬] 잠에서 깼다. (누나방)`
  - `busy`·사용자 정의 → `[씬] 하던 일을 마쳤다: {표시 라벨}. (안방화장실)`
    (라벨은 `_status_display_label`, 위치 미사용 시나리오는 괄호 생략)
- 도착지의 **다른 사람**에게 가는 "도착했다" 알림은 예전처럼 **다음 wave**다
  (scene_injections 버퍼의 초기값으로 이월).
- `agent_status_change`(clear)는 여전히 한 번, 같은 wave 번호로 나간다 — 다만 이제
  그 wave의 `wave_start`보다 먼저라, 라이브 피드는 디렉터 카드처럼 그 wave의
  구분선 뒤로 미뤄 붙인다(`feed.js::addStatusCard` → `_appendWaveCard`). 마크다운
  내보내기는 원래 clear를 "대사 전"으로 놓는다.
- **자연 만료만** 대상이다. 개입으로 턴을 받아 `enter_state`를 비워 즉시 해제되는
  경로(아래 "자기 상태 인지")는 이미 턴을 받은 것이라 알림을 추가하지 않는다.
- 전원 상태 잠금의 `wake_key` `[]` 재투입은 그대로 — 다음 wave 시작의 만료 처리가
  같은 키에 알림을 한 번 붙인다(중복 없음). 풀린 사람은 이번 wave 턴을 받으므로
  `_last_turn_wave`도 자연히 갱신된다(소외 재투입 기준).
- 재개 시 `remaining_minutes: 0`으로 복원된(저장 시점에 이미 끝난) 상태는 재개 첫
  wave 시작에 바로 풀려 알림 + 턴을 받는다. wave 도중 중지되면 이미 지워진 상태의
  "도착했다" 알림(다음 wave 몫)을 `_pending_wave`에 실어 재개 때 잃지 않는다.
...
1. **이동 중에도 이미 예약된 턴이 실행됨** — 상대가 말을 건 *그 wave*에 발신자가
   ...
   상태가 풀리는 wave에 되돌려준다(`_release_held_incoming` — 지금은 본인 해제
   알림 바로 뒤에 붙여 그 wave 턴의 incoming으로, 위 "자연 만료").
...
**"전원 재투입"의 상태 무시 — 위 관측성으로 실측한 5번째 사안** — 활성
에이전트 전원이 상태로 묶이면 idle 스케줄 대신 `_earliest_status_clear`로
가장 이른 해제 시점까지 정확히 점프하는 것까진 맞지만, 그 뒤 **전원**을
무조건 재투입해 아직 자기 해제 시점이 안 된 에이전트까지 매 침묵 사이클마다
거의 같은 잠꼬대를 다시 선언하게 만들었다 — v11 실행에서 신짱구·신짱아가
같은 밤 3~4번 연속으로 "드르렁... 슛... 골!!!"류를 재선언하는 게 마크다운
...
만료 처리(`_expire_agent_states`)로 자연히 합류한다(해제 알림과 함께 그 wave에
턴 — 위 "자연 만료"). 상태에 안 묶인
...
상태 중인지 확인하지 않고 "한 명이라도 상태 중이면" 그 사람의 해제 시점을
그대로 raw_jump로 썼다. 이 wave가 "전원 침묵"(아무도 서로를 못 닿음)으로
판정된 것과 활성 에이전트 전원이 상태 중인 것은 별개인데, 둘을 같은 조건으로
취급한 것이 원인이다. 실측 사례: 밤 10시대에 한 명(짱구)만 수면에 들고
다른 한 명(신짱아)은 막 씻기를 끝냈을 뿐 아직 안 잤는데, 짱구 한 명의 해제
시점(몇 시간 뒤)으로 온 가족의 시계가 그대로 점프해 신짱아의 남은 저녁
활동이 통째로 증발했다(22:53→03:38). 활성 에이전트 **전원**이 실제로 상태
중인지(`all(... for k in self.active_agents)`) 확인한 뒤에만 이 점프를
쓰도록 고쳤다 — 전원이 아니면 기존 `idle_minutes_schedule`(60/120/180분
등 작은 조각)로 그대로 진행하므로, 아무도 끝까지 잠들지 않는 극단적인
경우에도 막히지 않고 다음 "전원 침묵" 사이클마다 다시 판단한다.

**idle 점프 해제 시점 클램프(사용자 실측 7번째 사안).** 위 6번째 사안의 수정으로
"전원 잠금"일 때만 해제 시점까지 점프하게 됐는데, 반대로 **일부만** 상태 중이면
idle 스케줄(60/120/180분)이 그 사람의 해제 시점을 그냥 지나쳤다(실측: 씻기가
23:09에 끝나는데 22:46→23:46 점프). LLM/카테고리 경로의 `_clamp_time_jump`는 이미
"상태 해제 시점" 캡을 걸고 있었으므로, `idle_jump` 분기도 같은 원칙으로 예정
이벤트(beat)와 가장 이른 상태 해제 시점 중 이른 쪽에서 자른다(동률이면 예정
이벤트, 최소 1분). `time_jump`(mode=idle)의 `clamp_reason`에
`상태 해제 시점 전까지 60→23분` / `예정 이벤트(HH:MM) 전까지 …`로 남는다. 해제
시점에서 멈추면 다음 wave 시작의 만료 처리가 그 사람에게 알림 + 턴을 준다.
...
**두 번째 독립 리뷰(2026-09-22) 보완 — 시간 캡 통합 + 이동 중 강제 편입 차단.**
같은 v11 실행을 다시 코드 재현으로 검증한 리뷰가 추가로 2건을 확인했다(자기
상태 인지 부재로 인한 나머지 항목은 더 큰 설계 논의가 필요해 별도 보류):

1. **`_clamp_time_jump`가 여러 상한 중 최솟값이 아니라 먼저 걸리는 조건에서
   바로 반환했다** — 예정 이벤트 캡이 상태 해제 캡보다 먼저 검사돼, 상태
   해제가 더 급해도(예: 10분 뒤) 예정 이벤트가 더 느슨하면(예: 60분 뒤)
   그 60분이 그대로 통과해버렸다. 예정 이벤트·상태 해제·목표 기간·동석
   장면·주간 상한을 전부 `(분, 사유)` 후보로 모아 마지막에 `min()`을 한 번만
   취하도록 재구성했다. 이 김에 **목표 기간(target_duration_minutes) 마감도
   새 캡으로 추가** — 기존엔 이 캡이 아예 없어서 목표를 채운 뒤에도 자유
   점프가 몇 시간 더 지나쳐야 다음 wave의 종료 체크가 뒤늦게 걸렸다(2,880분
   목표가 2,995분에야 종료된 사례).
2. **예약 이벤트·디렉터 개입이 traveling 체크 없이 current_wave에 직접
   꽂혔다** — `next_wave` 구성 시점의 hold 처리(위 리뷰 1번 수정)는 그
   지점 하나만 지킨다. 그런데 예약 이벤트(`entrant`/`notified`, events.py)와
   디렉터 개입(`_run_system_agent`, system.py)은 그보다 앞서 current_wave를
   직접 편집하며 이 체크를 거치지 않았다 — 그래서 이동 중인데 "18:00 학원
   끝났다" 같은 예정 알림이 그대로 꽂혀 그 wave에 아직 도착 안 한 곳에서
   활동하는 서사가 실제 실행(W82)에서 확인됐다. 사이트별로 패치하는 대신
   **실제 LLM 호출(turn_executor.submit) 바로 직전에 공통 관문**을 하나
   추가했다 — 그 시점 current_wave에 남은 traveling 에이전트를 전부
   `_hold_incoming`으로 넘기고 제외한다. 알림의 memory 기록 자체(사실
   전달)는 이 필터와 무관하게 그대로 이뤄지므로, 도착 후 자연스러운 계기에
   반응할 근거는 남는다. 활성 에이전트 전원이 동시에 이동 중인 극단적
   경우(현실적으로 거의 없음)는 이 필터를 건너뛰어 진행 불가를 피한다.

리뷰가 지적한 나머지 두 건(자기 선언 상태의 조기 종료·재선언 시 타이머
리셋)은 근본적으로 "에이전트가 자기 상태 정보를 프롬프트에서 전혀 못 본다"는
같은 원인으로 묶이고, 프롬프트 계약 확장이 필요해 별도 논의 후 진행했다 —
아래.

**자기 상태 인지 + 개입에 의한 자연스러운 해제(사용자 설계).** 지금까지
상태 해제는 오직 엔진(시간)만 결정했다 — 에이전트가 서사로 "다 씻었다"고
말해도 `_agent_status`는 그대로 남아, 서사와 엔진 상태가 어긋날 수 있었다
(리뷰 2·3번). 사용자 제안: "다른 에이전트의 개입으로 상태가 해제되는 것이
자연스럽다 — 엄마가 깨우면 세수하며 실제로 깨어난다"는 관찰에서 출발해,
다음 세 가지로 구현했다:

1. **자기 상태 노출** — `[현재 상황]`(`_build_situation_context`,
   location.py)에 활성 상태가 있으면 한 줄 추가: "당신은 지금 'OO' 상태입니다
   (약 N분 뒤 HH:MM에 자연히 끝날 예정)." `traveling`은 제외한다 — 이미 별도
   "이동 중" 줄이 있고(경로 hop 수, 다른 개념), 애초에 `_same_room`이 누구의
   개입도 막아버려 유지/해제를 스스로 판단할 여지가 없는 순수 엔진 제어이기
   때문이다(사용자 지적: "이동 상태는... 다른 에이전트의 개입이 불가능하지
   않을까"). `_assemble_agent_prompt`(step.py, 실행/읽기전용 조회 공통 경로)가
...
2. **재선언 없으면 즉시 해제** — `enter_state` 필드를 "이번 턴에도 이 상태를
   ...
   (`runner.py`, enter_state 처리 블록). `traveling`은 이 판단에서 제외 —
   자기 상태 노출과 같은 이유. 상태가 없던(가장 흔한 경우) 에이전트는
   조용히 넘어간다.
3. **계약 규칙 추가** — `build_state_hint`(prompt_contract.py)에 "상태 중
   개입으로 턴을 받으면 유지 여부를 스스로 판단하세요 — 계속 유지하려면
   같은 id를 다시, 벗어났다면 null을 적으세요(즉시 해제)" 규칙을 추가해
   위 두 메커니즘을 에이전트가 이해하도록 명시했다.

**재선언 시 타이머 유지(사용자 설계, 후속 진행).** `_enter_state`(status.py)의
자기-선언형 경로에 판단 하나를 추가했다: 지금 선언하려는 category가 **이미
활성 중인 상태와 같은 id**면 새로 뽑지 않고 `None`을 반환해 기존
`until_elapsed`를 그대로 둔다(호출부는 반환값이 `None`이면 "enter" 이벤트를
다시 emit하지 않는다 — 변화가 없으므로). 타이머가 실제로 만료된 뒤(또는
애초에 상태가 없었을 때) 다시 진입하거나, **다른** category로 전환하는
경우는 기존처럼 새로 뽑는다. 사용자 설계 그대로다: "이전 타이머가 남아 있는
상태에서... 타이머 변경 없이 이어서 진행하고, 타이머가 끝나서 다시 상태에
진입한다고 하면 그 때 다시 타이머를 뽑는다." 판단 기준이 `_agent_active_status`
호출 하나뿐이라 수정 범위는 `_enter_state` 안 몇 줄로 끝난다.
