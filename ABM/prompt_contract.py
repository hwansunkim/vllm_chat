...
**이번 턴의 행동 하나만** 담은 JSON 객체를 **정확히 하나** 출력합니다. 여러 단계를
미리 나열하거나 객체를 여러 개 이어 붙이지 마세요. 코드블록(```) 없이 순수 JSON만.
{
...
⚠ JSON 객체는 이번 턴의 것 **하나만** 출력하십시오. 다음 턴 몫까지 미리 쓰지 마십시오.
⚠ [현재 상황]의 '이 자리의 사람들'에 없는 사람이 방금 왔다거나 지금 곁에 있다고
상상해서 서술하지 마십시오. 그 사람이 실제로 그 목록에 나타난 뒤에만 상호작용을
서술할 수 있습니다.
"""
...
def build_state_hint(state_categories: list[dict] | None = None) -> str:
    ...
    return (
        ...
        "예외입니다.\n"
        "  ※ 상태 중 개입으로 턴을 받으면 유지 여부를 스스로 판단하세요 — 계속 "
        "유지하려면 같은 id를 다시, 벗어났다면 null을 적으세요(즉시 해제)."
    )
...
def build_infection_contract(
    *, infection_enabled: bool = False, disease_name: str = ""
    ...
    감염 상태(S/E/I/R)·확률·경과 시간 같은 raw 값은 **절대** 프롬프트에 넣지 않는다
