...
배경: 재투입(휴면 처리)이 에이전트의 상태를 전혀 모른 채 무조건 다시 초대해서,
...
3. "전원 재투입" 시간 점프(runner.py) — 활성 에이전트 전원이 상태에 묶여 있으면
   idle 스케줄의 랜덤값 대신 **가장 이른 상태 해제 시점까지 정확히 점프**하고,
   그 시점에 실제로 풀리는 에이전트 **한 명만** 다시 초대한다 — 아직 안 풀린
   나머지를 같이 깨우면 "아직 자는 중"이라는 거의 같은 대사를 반복하게 될
   뿐이다. 나머지는 각자의 해제 시점에 다음 wave 상단의 만료 처리
   (`_expire_agent_states`)로 자연히 합류한다.
"""
import random
